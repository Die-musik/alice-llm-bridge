...
### Автоматический возврат длинного ответа на колонку

Опциональный `runtime.voice_return` убирает фразу «скажите готово» для явно
сопоставленных колонок. Если Codex не уложился в webhook-бюджет, навык быстро
говорит «Секунду.» и закрывает текущую сессию. После сохранения готового ответа
bridge запускает отдельный сценарий Яндекса на той же колонке; сценарий снова
открывает навык, тот забирает уже сохранённый ответ по своему `application_id`,
произносит его и оставляет микрофон в режиме слушания.

Сопоставление всегда явное: `application_id` поверхности и Quasar `device_id`
берутся из отдельных переменных окружения. X-token аккаунта тоже передаётся
только через окружение. При старте bridge создаёт либо приводит к точному виду
один именованный сценарий на каждую настроенную колонку. Если отправка сценария
не удалась, готовый ответ не теряется: пользователь может снова открыть навык и
сказать «готово». Неофициальный Quasar API остаётся сознательным ограничением;
при его изменении отключите `voice_return.enabled`, не меняя основной навык.
Web-сессия Яндекса создаётся один раз при старте процесса. Пока не зафиксирован
хотя бы один случай её истечения между штатными перезапусками, восстановление —
перезапуск bridge; после первого такого случая upgrade path — одна
сериализованная повторная авторизация и ровно один повтор запроса.

Пример конфигурации находится в `config.example.toml`. До включения задайте
`YANDEX_STATION_X_TOKEN`, `ALICE_LIVING_ROOM_APPLICATION_ID` и
`YANDEX_LIVING_ROOM_DEVICE_ID` в закрытом environment-файле сервиса. Homey к
этому каналу не относится и остаётся выключенным.
